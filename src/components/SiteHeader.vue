  ...
  <header>
    <div class="wrapper">
      ...
      <nav class="main-navigation">
        <NavbarLink v-for="link in links" :link="link" :key="`${link.name}-main-navigation`" />
      </nav>
      <div class="actions">
        <button>
        ...
        <svg class="md:hidden block ham hamRotate ham1" viewBox="0 0 100 100" width="45" onclick="this.classList.toggle('active')">
...
header {
  .wrapper {
    @apply container mx-auto md:mt-5 h-[100px] flex justify-between items-center;
    .logo {
      @apply ml-3 md:ml-0 font-didact text-3xl text-primary w-full md:w-[145px];
    }
    .main-navigation {
